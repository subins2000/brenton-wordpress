...
/**
 * This function converts headings with id attribute to links for reference
 *
...
function anchor_content_headings($content) {
    ...
    $content = preg_replace_callback("/\<h([1|2|3])\>\<span id=\"(.*?)\"\>(.*?)\<\/span\>\<\/h([1|2|3])\>/", function ($matches) {
      $hTag = $matches[1];
      $slug = $matches[2];
      $title = $matches[3];
      return '<h'. $hTag .'><a href="#'. $slug .'"><span id="' . $slug . '">' . $title . '</span></a></h'. $hTag .'>';
    }, $content);
    return $content;
}
add_filter('the_content', 'anchor_content_headings', 100);
